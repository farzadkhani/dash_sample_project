...
app.layout = html.Div(children=[
    ...
    html.Div(
        [
            "Input Year: ", 
            dcc.Input(
                id='input-year', value='2010', type='number', style={'height':'35px', 'font-size': 30}
            ),
        ],
        style={'font-size': 30}
    ),
...
def compute_data(airline_data, entered_year):
    ...
    avg_delay = df.groupby('Month')[
        ['CarrierDelay', 'WeatherDelay', 'NASDelay', 'SecurityDelay', 'LateAircraftDelay']
    ].mean().reset_index()
    # New Data for Query Table
    query_result = df.groupby(['Month', 'Reporting_Airline'])[
        ['CarrierDelay', 'WeatherDelay', 'NASDelay', 'SecurityDelay', 'LateAircraftDelay']
    ].mean().reset_index()
...
def get_graph(entered_year):
    ...
    return[
        carrier_plot, weather_plot, NAS_plot, security_plot, late_plot, avg_delay_plot, table, query_table
    ]
